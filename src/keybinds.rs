...
impl TryFrom<&KeyEvent> for KeybindAggregate {
    ...
    fn try_from(key: &KeyEvent) -> Result<Self, Self::Error> {
        ...
        match key.code {
            ...
            KeyCode::Char('b') if ctrl => Ok(Self::Left),

            KeyCode::Right => Ok(Self::Right),
            KeyCode::Char('l') if !ctrl => Ok(Self::Right),
            KeyCode::Char('f') if ctrl => Ok(Self::Right),
...
mod tests {
    ...
    #[test]
    fn left_set_matches_arrow_h_and_ctrl_b() {
        assert_eq!(
            KeybindAggregate::try_from(&key(KeyCode::Left, KeyModifiers::NONE)),
            Ok(KeybindAggregate::Left)
        );
        assert_eq!(
            KeybindAggregate::try_from(&key(KeyCode::Char('h'), KeyModifiers::NONE)),
            Ok(KeybindAggregate::Left)
        );
        assert_eq!(
            KeybindAggregate::try_from(&key(KeyCode::Char('b'), KeyModifiers::CONTROL)),
            Ok(KeybindAggregate::Left)
        );
    }

    #[test]
    fn right_set_matches_arrow_l_and_ctrl_f() {
        assert_eq!(
            KeybindAggregate::try_from(&key(KeyCode::Right, KeyModifiers::NONE)),
            Ok(KeybindAggregate::Right)
        );
        assert_eq!(
            KeybindAggregate::try_from(&key(KeyCode::Char('l'), KeyModifiers::NONE)),
            Ok(KeybindAggregate::Right)
        );
        assert_eq!(
            KeybindAggregate::try_from(&key(KeyCode::Char('f'), KeyModifiers::CONTROL)),
            Ok(KeybindAggregate::Right)
        );
    }
}
